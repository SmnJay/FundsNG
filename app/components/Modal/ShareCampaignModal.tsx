...
const ShareCampaignModal: React.FC<Props> = ({ shareableUrl, pushToUrl, showModal, handleModal }) => {
    const router = useRouter();

    const handleCopyClick = async () => {
        try {
            await navigator.clipboard.writeText(shareableUrl as string);
        ...
        >
            ...
            <div className="rounded-lg my-4 bg-[#EBF7DF] border border-[#7D847C] py-2 px-3 flex items-center justify-between">
                <div className="">
                    ...
                    <span className="text-FBlack text-sm font-medium block leading-loose">{truncateLink(shareableUrl)}</span>
                </div>
